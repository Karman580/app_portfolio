...
export default function KeyDifferentiators() {
  return (
    <section id="features" className="relative w-full bg-base py-24 border-b border-border-main grid-bg">
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="text-accent-cyan font-semibold text-xs tracking-wider uppercase mb-3 flex items-center justify-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-accent-cyan"></span> Advanced Core Heuristics
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {differentiators.map((diff, idx) => {
            ...
            return (
              <div 
                key={idx}
                className={`glass-panel glass-panel-hover p-8 rounded-2xl border border-border-main transition-all duration-300 ${
                  isLarge ? 'md:col-span-2' : ''
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className={`p-3 rounded-xl bg-base-muted/50 ${diff.color}`}>
                ...
                <h3 className="text-lg font-bold text-text-primary mt-6 mb-2 tracking-tight">
                  {diff.title}
                </h3>
                <p className="text-sm text-text-muted/80 font-light leading-relaxed">
                  {diff.desc}
                </p>
              </div>
            );
